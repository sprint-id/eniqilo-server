...
func (h *transactionHandler) Checkout(w http.ResponseWriter, r *http.Request) {
	...
	res, err = h.transactionSvc.AddTransaction(r.Context(), req, token.Subject())
	if err != nil {
...
func (h *transactionHandler) GetTransactionHistory(w http.ResponseWriter, r *http.Request) {
	...
	// show request
	fmt.Printf("Transaction History request: %+v\n", param)
	...
	res, err = h.transactionSvc.GetTransactionHistory(r.Context(), param, token.Subject())
	if err != nil {
		code, msg := ierr.TranslateError(err)
		http.Error(w, msg, code)
		return
